...
export class SegmentedControlElement extends LitElement {
  ...
  static override styles = [segmentedControlContainerStyle, segmentedControlItemStyle]
  ...
  protected override async firstUpdated() {
    ...
    subscribe(this, [
      ...
      this.parentElement
        ? resizeObserver(this.parentElement).pipe(tap(() => this.updated()))
        : of(),
    ])
  }
  ...
  protected getItemView(item: SegmentedControlItem) {
    ...
    if (item.template) {
      templateRef = item.template(item, item.value === this.select?.value, this.caretRef)
    }
    return html`
      <div
        @click="${(event: MouseEvent) => this.onSelect(item, event)}"
