import type { ICacheAsync } from '@1inch-community/models'
...
async function getDexie<Key, Value>(
  storageKey: string
): Promise<Table<LongTimeCacheData<Key, Value>, Key>> {
  const Dexie = await import('dexie').then((m) => m.Dexie)
  const db = new Dexie('one-inch-long-time-async-cache_' + storageKey)
  db.version(version).stores({
    cache: ['key', 'value', 'timestamp'].join(', '),
  })
  return Reflect.get(db, 'cache')
}

export class LongTimeAsyncCache<Value, Key extends string = string>
  implements ICacheAsync<Key, Value>
{
  ...
  async get(key: Key): Promise<Value | null> {
    ...
    const result = await db
      .filter((item) => Date.now() - item.timestamp < this.ttlDays * 8.64e7 && item.key === _key)
      .first()
    this.cleanOldRecords().catch(console.error)
  ...
  async getAll() {
    const db = await this.getDexie(this.storageKey)
    const result = await db
      .filter((item) => Date.now() - item.timestamp < this.ttlDays * 8.64e7)
      .toArray()
    this.cleanOldRecords().catch(console.error)
  ...
  private async cleanOldRecords() {
    ...
    await db
      .where('timestamp')
      .below(Date.now() - this.ttlDays * 8.64e7)
      .delete()
  }
