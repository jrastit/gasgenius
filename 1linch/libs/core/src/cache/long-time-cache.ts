import { ICache } from '@1inch-community/models'
import { JsonParser, storage } from '../storage'
...
export class LongTimeCache<Key, Value> implements ICache<Key, Value> {
  ...
  constructor(
    ...
    const data = storage.get<[Key, LongTimeCacheData<Value>][]>(
      this.storageKeyAndVersion,
      JsonParser
    )
  ...
  private checkCacheData(key: Key, skipUpdatePersistence?: boolean): boolean {
    ...
    let isDirty = false
    if (Date.now() - data.timestamp > this.ttlDays * 8.64e7) {
