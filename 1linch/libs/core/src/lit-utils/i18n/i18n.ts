import { Locale, Translations } from '@1inch-community/models'
...
export const localeChange$ = currentTranslations$.pipe(map(() => currentLocaleCode))
...
export async function addTranslation(
  translationsRecord: Record<Locale, () => Promise<Translations>>
) {
...
export function listenChangesByPath(
  path: string,
...
const translationsListToRecord = (translationsList: Translations[]): Translations =>
  translationsList.reduce((record, translations) => ({ ...record, ...translations }), {})

async function updateLocaleAndTranslations(newLocaleCode: Locale) {
  targetHost.dir = isRTL(newLocaleCode) ? 'rtl' : 'ltr'

  await Promise.all([updateDefaultTranslationsMap(), updateCurrentTranslationsMap(newLocaleCode)])
...
async function updateCurrentTranslationsMap(newLocaleCode: Locale) {
  ...
  if (
    currentLocaleCode !== newLocaleCode ||
    translationsByLocale.length !== lastCurrentTranslationsStorageSize
  ) {
