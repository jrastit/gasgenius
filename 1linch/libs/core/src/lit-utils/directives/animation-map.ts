...
> {
  ...
  onAfterRemoveAnimateItem?(
    element: HTMLElement,
    index: number,
    previousStepState: void | RemoveState
  ): Promise<void>

  onBeforeRenderAnimateItem(element: HTMLElement, index: number): Promise<RenderState>
  onAfterRenderAnimateItem?(
    element: HTMLElement,
    index: number,
    previousStepState: void | RenderState
  ): Promise<void>

  onBeforeMoveAnimationItem?(
    element: HTMLElement,
    oldPosition: number,
    newPosition: number
  ): Promise<MoveState>
  onAfterMoveAnimationItem?(
    element: HTMLElement,
    previousStepState: void | MoveState
  ): Promise<void>
...
export class AnimationMap<Value> extends Directive {
  ...
  private async transition(items: Iterable<Value>, controller: AnimationMapController<Value>) {
    ...
    const renderCandidateElements = this.getAllRenderCandidateElements(
      renderCandidate,
      keyMap,
      controller
    )
    ...
    const [, renderResultState, removeResultState, moveResultState] = await Promise.all([
      controller.onBeforeAnimation?.(
        ...
        removeCandidateNodeIndexMap,
        moveCandidateNodeIndexMap
      ),
      this.executeHandlerAndReturnResultMap(renderCandidateNodeEntries, (node, key) =>
        controller.onBeforeRenderAnimateItem(node, indexMap.get(key)!)
      ),
      this.executeHandlerAndReturnResultMap(removeCandidateNodeEntries, (node, key) =>
        controller.onBeforeRemoveAnimateItem(node, viewIndexMap.get(key)!)
      ),
      this.executeHandlerAndReturnResultMap(
        moveCandidateNodeEntries,
        (node, key) =>
          controller.onBeforeMoveAnimationItem?.(
            node,
    ...
    await Promise.all([
      this.executeHandlerAndReturnResultMap(
        renderCandidateNodeEntries,
        (node, key) =>
          controller.onAfterRenderAnimateItem?.(
            node,
            indexMap.get(key)!,
            renderResultState.get(key)
          ) ?? Promise.resolve()
      ),
      this.executeHandlerAndReturnResultMap(
        removeCandidateNodeEntries,
        (node, key) =>
          controller.onAfterRemoveAnimateItem?.(
            node,
            viewIndexMap.get(key)!,
            removeResultState.get(key)
          ) ?? Promise.resolve()
      ),
      this.executeHandlerAndReturnResultMap(
        moveCandidateNodeEntries,
        (node, key) =>
          controller.onAfterMoveAnimationItem?.(node, moveResultState.get(key)) ?? Promise.resolve()
      ),
      controller.onAfterAnimation?.(
        this.view,
        renderCandidateNodeIndexMap,
        removeCandidateNodeIndexMap,
        moveCandidateNodeIndexMap
  ...
  private getAllRenderCandidate(
    indexMap: Map<string, number>,
    newIndexMap: Map<string, number>
  ): Set<string> {
  ...
  private getAllRenderCandidateElements(
  ...
  private moveNode(
    candidateElements: Map<string, HTMLElement>,
  ...
  private getKeyMapAndIndexMap(
    items: Iterable<Value>,
    controller: AnimationMapController<Value>
  ): [Map<string, Value>, Map<string, number>] {
