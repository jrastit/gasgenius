...
export class ProviderDataAdapter implements IDataAdapter, IProviderDataAdapterInternal {
  private context = lazyAppContext('ProviderDataAdapter')
  ...
  readonly addresses$: Observable<Address[]> = this.providerOrDisconnect$.pipe(
    switchMap((provider) => {
      ...
      return from(this.getAddresses()).pipe(
        switchMap((addresses) =>
          fromEIP1193Event(provider, 'accountsChanged').pipe(startWith(addresses))
        )
      )
    }),
  ...
  readonly chainId$: Observable<ChainId | null> = this.providerOrDisconnect$.pipe(
    switchMap((provider) => {
      ...
      return from(this.getWalletChainId()).pipe(
        switchMap((chainId) =>
          fromEIP1193Event(provider, 'chainChanged').pipe(
            map((chainStr) => {
  ...
  readonly isConnected$ = this.activeAddress$.pipe(map((address) => !!address))
