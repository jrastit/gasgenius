...
import { LongTimeAsyncCache } from '@1inch-community/core/cache'
import { getMobileMatchMedia } from '@1inch-community/core/lit-utils'
import { INotificationsManager, NotificationConfig } from '@1inch-community/models'
...
export class NotificationsManager
  implements INotificationsControllerInternal, INotificationsManager
{
  ...
  async error(template: string | TemplateResult): Promise<string> {
    return await this.show('Error', template, { errorStyle: true })
  }

  async warning(template: string | TemplateResult): Promise<string> {
    return await this.show('Warning', template, { warningStyle: true })
  }
  ...
  private getNotificationsContainer() {
    ...
    if (this.mobileMatchMedia.matches) {
      container = document.createElement(
        NotificationsMobileContainerElement.tagName
      ) as NotificationsContainer
    } else {
      container = document.createElement(
        NotificationsDesktopContainerElement.tagName
      ) as NotificationsContainer
    }
