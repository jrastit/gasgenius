...
import {
  NotificationsBaseContainerElement,
  notificationsBaseContainerStyle,
} from '../notifications-base-container'
...
export class NotificationsMobileContainerElement extends NotificationsBaseContainerElement {
  ...
  protected firstUpdated() {
    subscribe(this, [this.onShowHideUpButton(), this.swipeDownOnShortViewHandler()], {
      requestUpdate: false,
    })
  }
  ...
  async preClose(): Promise<void> {
    ...
    if (!this.fullView) {
      ...
      await Promise.all([
        ...
        this.touchContainerRef.value!.animate([{ opacity: 1 }, { opacity: 0 }], {
          ...animationOptions,
          duration: 1000,
        }).finished,
        this.backgroundViewRef.value!.animate(
          [getBackdropFilter('blur(3px)'), getBackdropFilter('blur(0px)')],
