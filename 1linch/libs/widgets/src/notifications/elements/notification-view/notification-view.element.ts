import { appendStyle, dispatchEvent, subscribe } from '@1inch-community/core/lit-utils'
...
export class NotificationViewElement extends LitElement {
  ...
  connectedCallback() {
    ...
    subscribe(this, [this.onTouch()], { requestUpdate: false })
  }
      ...
      <div class="${classMap(classes)}">
        <div>
          <inch-button
            ...
            @click="${() => dispatchEvent(this, 'closeNotification', null)}"
            size="l"
            type="secondary"
          >
      ...
      switchMap((startEvent) => {
        ...
        return fromEvent<TouchEvent>(document, 'touchmove').pipe(
          tap((event) => {
            ...
            if (Math.abs(distance) > window.innerWidth * 0.3) {
              this.onClose(distance).catch()
            }
          }),
          takeUntil(
            fromEvent(document, 'touchend').pipe(switchMap(() => this.onResetPosition(distance)))
          )
        )
      })
    )
  ...
  private formatNotificationTime(timestamp: number): string {
    ...
    if (Date.now() - timestamp > 6 * 60 * 60 * 1000) {
      const dateView = [formatTimeValue(date.getDate()), formatTimeValue(date.getMonth() + 1)].join(
        '.'
      )
