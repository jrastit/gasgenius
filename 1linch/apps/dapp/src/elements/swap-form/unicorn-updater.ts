...
export function unicornTouchUpdate(
  context: IApplicationContext,
  swapFormContainerRef: Ref<HTMLElement>,
  unicornLoaderRef: Ref<HTMLElement>
) {
  let lastPosition = 0
  let full = false
    ...
    return fromEvent<TouchEvent>(targetEl, 'touchstart').pipe(
      ...
      switchMap((startEvent: TouchEvent) => {
        ...
        return fromEvent<TouchEvent>(targetEl, 'touchmove').pipe(
          ...
          takeUntil(fromEvent<TouchEvent>(targetEl, 'touchend').pipe(tap(() => reset())))
        )
      })
    )
